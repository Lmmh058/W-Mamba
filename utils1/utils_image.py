...
def imread_uint(path, n_channels=1):
    ...
    if n_channels == 1:
        ...
        img = cv2.imread(path, cv2.IMREAD_COLOR)  # Read in BGR color format
        ...
        y_channel = img_yuv[:, :, 0:1]  # Y channel as HxWx1
        uv_channels = img_yuv[:, :, 1:3]  # UV channels as HxWx2

        return y_channel, uv_channels  # Return both Y and UV channels
...
def tensor2uint(img):
    img = img.data.detach().squeeze().float().clamp_(0, 1).cpu().numpy()
    if img.ndim == 3:
